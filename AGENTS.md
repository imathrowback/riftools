...
- **TelaraDBDiff invocation**: `java -jar telaradbdiff/build/jar/telaradbdiff.jar -dbA <fileA> -dbB <fileB> -dbResolve /tmp/rift_datamodel.txt [-langA <cdsA> -langB <cdsB>] -outdir <dir> -format html -output <path>`
  - Or via the main CLI: `java -jar RiftTools/build/jar/RiftTool.jar -action TELARADBDIFF -dbA <fileA> ...` (if the action exists)`
...
- **ToTextMode invocation**: `java -jar totext/build/jar/totext.jar -fileType CDS -file <input.cds> -output <output.txt>`. Converts `.cds` localization databases to text format for text-level diffs.
