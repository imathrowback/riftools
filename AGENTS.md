...
- **Running RiftTool**: Use the full absolute path: `java -jar ~/riftools/RiftTools/build/libs/RiftTool.jar <args>`. Run without args to see usage. Available actions include `PRINTVERSIONS`, `GETVERSION`, `DOWNLOADFILE`, `EXTRACT`, etc.
...
- **Downloading files for TelaraDBDiff**: Use `java -jar RiftTools/build/libs/RiftTool.jar -action DOWNLOADFILE -pIndex N -release LIVE -filename <name> -outfilename <path>`. Do NOT pass `-lang 0` — that restricts to lang=0 entries which may not match English (lang=1). Omit the `-lang` flag entirely (defaults to `-1` = any language). Key files to download:
...
- **TelaraDBDiff invocation**: `java -jar telaradbdiff/build/libs/telaradbdiff.jar -dbA <fileA> -dbB <fileB> -dbResolve /tmp/rift_datamodel.txt [-langA <cdsA> -langB <cdsB>] -outdir <dir> -format html -output <path>`
  - Or via the main CLI: `java -jar RiftTools/build/libs/RiftTool.jar -action TELARADBDIFF -dbA <fileA> ...` (if the action exists)`
...
- **ToTextMode invocation**: `java -jar totext/build/libs/totext.jar -fileType CDS -file <input.cds> -output <output.txt>`. Converts `.cds` localization databases to text format for text-level diffs.
